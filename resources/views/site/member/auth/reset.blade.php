                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="form-login text-center">
                                {!! Form::open(['route' => 'site.member.reset', 'method' => 'post', 'data-toggle' => 'validator', 'id' => 'login-form']) !!}
                                    ...
                                    <div class="form-group {{ $errors->has('password_confirmation') ? 'has-error' : null }} has-feedback">
                                        {!! Form::password('password-confirm', ['placeholder' => 'CONFIRME A SENHA', 'required', 'id' => 'password-confirm', 'class' => 'form-control']) !!}

                                        <div class="help-block with-errors pull-right"></div>
                                    </div>

                                    {!! Form::hidden('token', $token) !!}
                                    <button type="submit" class="btn text-uppercase btn-link btn-white">Redefinir senha</button>
                                {!! Form::close() !!}
                            </div>
                        </div>
                    </div>
...
<script>
    $(function () {
        ...
        $(window).on('resize', function(){
            ...
            sectionLogin.find('.container').css({'marginTop': $(window).height() / 6, 'marginBottom': $(window).height() / 6});
        });
